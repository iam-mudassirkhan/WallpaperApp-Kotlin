...
import com.mudassir.wallpaper.Adapters.ArtWallpaperAdapter
...
class ArtWallpaperFragment : Fragment(), WallpaperOnClickListener {
  private var binding: FragmentArtWallpaperBinding? = null
    ...
    override fun onCreateView(
        ...
        binding = FragmentArtWallpaperBinding.inflate(layoutInflater, container, false)
        return binding?.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        ApiClientWithOutMVVM().getClient()?.getArtWallpapers()?.enqueue( object :
    ...
    fun setUpRecyclerView(wallpaperList: ArrayList<com.mudassir.wallpaper.Models.Dir>) {
        binding?.artWallpaperRV?.layoutManager = GridLayoutManager(requireContext(), 3)
        binding?.artWallpaperRV?.adapter = ArtWallpaperAdapter(requireContext(), wallpaperList, this)
    }
